...
sensitivity_graph_both_sj <-  sensitivity_graph_template %+% sensitivity_calibration_sj_pivoted +
	...
	geom_errorbar(aes(ymax=singsi.uci, ymin=singsi.lci), width=0.2, linewidth=1, color="black") +
	geom_errorbar(aes(ymax=totsi.uci, ymin=totsi.lci), width=0.2, linewidth=1, color="#888888") +
...
validation_calibration_prior <- sir_calibration$model_return_prior %>%
	...
	bind_rows %>%
	mutate(distribution = "prior_calibration",
...
validation_calibration_posterior <- sir_calibration$model_return_posterior %>%
	purrr::map(~validation(.)$validation) %>%
	bind_rows %>%
	mutate(distribution = "posterior_calibration",
				 driver = experiment_driver,
				 value = paste(experiment_driver_values, collapse=","),
				 gsa_sample_n = gsa_sample_size,
				 sir_sample_n = sir_sample_size,
...
validation_violin_graph <- ggplot(validation_calibration, aes(y=rmse_stocks, x=distribution)) +
	geom_violin() +
	theme_bw()
