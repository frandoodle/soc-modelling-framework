...
site_data_rotationmean <- site_data_intercropsum %>%
	group_by_all %>%
...
Remove instances of overlapping sampling depths:
```{r}
site_data_filtered_removedsites %>%
	group_by(TrtID, replication_number, block, year_name) %>%
	filter(any(duplicated(soil_depth_min_cm)) | any(duplicated(soil_depth_max_cm))) %>%
	filter(ifelse(length(is.na(soc_tha)) >= 1, !is.na(soc_tha), TRUE))
```

Get the total SOC down to lowest sampling depth by summing
```{r}
