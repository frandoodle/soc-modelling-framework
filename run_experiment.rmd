...
experiment_driver_values <- c("Ellerslie", "Harrow", "Scott", "Lethbridge")
...
# Read site data
site_data <- readr::read_csv(here("data","LTE_Master_beta_mar29_addedcolumns.csv"), guess_max = Inf) %>%
...
(If there are multiple fields where each sequence of the rotation is being grown for the same year, we get the average yield/residue across each field/crop. If crops are grown on the SAME field, then we get the sum (e.g. cover crops))
...
# Handle intercropping (same field)
site_data_intercropsum <- site_data_filtered %>% 
	group_by_all %>%
	ungroup(!!current_crop_columns, !!residue_input_columns, !!fertilizer_columns) %>%
	summarise(across(current_crop_columns, first),
						across(residue_input_columns, sum),
						across(fertilizer_columns, sum))

# Handle rotation sequences
site_data_rotationmean

site_data_intercropsum %>% 
	group_by_all %>%
	ungroup(plot_id, !!current_crop_columns, !!residue_input_columns, !!fertilizer_columns) %>%
	summarise(across(current_crop_columns, first),
						across(residue_input_columns, mean),
...
offending_sites <- site_data_rotationmean %>%
...
site_data_filtered_removedsites <- site_data_rotationmean %>%
...
ribbon_points_posterior <- uncertainty_calibration_posterior$montecarlo %>%
	...
	select(year, treatment_name, soc_total_tha) %>%
...
ggplot(data = NULL, aes(x=year)) +
	geom_ribbon(data=ribbon_points_prior, aes(ymax=soc_total_max, ymin=soc_total_min), fill="lightgrey") +
	geom_ribbon(data=ribbon_points_posterior, aes(ymax=soc_total_max, ymin=soc_total_min), fill="darkgrey") +
	geom_point(data=uncertainty_calibration_posterior$montecarlo, aes(y=soc_tha_30cm), color = "black") +
	geom_line(data=uncertainty_calibration_posterior$median, aes(y=soc_total_tha), color = "black") +
	facet_wrap(vars(treatment_name)) +
	theme_bw()
