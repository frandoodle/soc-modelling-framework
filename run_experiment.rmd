...
validation_calibration <- validation_calibration_prior %>%
	bind_rows(validation_calibration_posterior)
```
...
ribbon_points <- uncertainty_calibration_posterior$montecarlo %>%
	group_by(year) %>%
	select(year, soc_total) %>%
	summarise(soc_total_max = max(soc_total),
						soc_total_min = min(soc_total))

ggplot(data = NULL, aes(x=year)) +
	geom_ribbon(data=ribbon_points, aes(ymax=soc_total_max/10, ymin=soc_total_min/10), fill="grey") +
