...
gorby <- climate_data %>%
	group_by(POLYID) %>%
	summarise(first_climate_year = first(Year))
	

site_data_raw %>%
	mutate(POLYID = as.character(POLYID)) %>%
	group_by(Exp_ID, POLYID) %>%
	summarise(location_name = first(location_name),
		n = n(),
						n_soc_tha = sum(!is.na(soc_tha)),
						n_crop_residue_kgha = sum(!is.na(crop_residue_kgha),
																			!is.na(hay_residue_kgha)),
						first_site_year = first(year_name)) %>%
	left_join(gorby, by="POLYID") %>%
	select(-POLYID) %>%
	write_csv(file="ExpID_summary.csv")
...
site_data_raw %>%
	...
	summarise(location_name = first(location_name),
						...
						n_crop_residue_kgha = sum(!is.na(crop_residue_kgha),
																			!is.na(hay_residue_kgha))) %>%
	write_csv(file="TrtID_summary.csv")
