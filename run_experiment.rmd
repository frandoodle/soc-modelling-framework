...
# List of experiments to remove because there are issues with them currently
removed_sites <- c("E10", "E11", "E12", "E17", "E29", "E53", "E58", "E61", "E64", "E67", "E72")

site_data_filtered_removedsites <- site_data_filtered_lethbridge %>%
	group_by(TrtID, replication_number, block, year_name) %>%
	filter(!Exp_ID %in% removed_sites) %>%
	group_split()

# Get each possible combination of sampling depths to reach 0-30cm for each experiment
site_data_depthgroups <- site_data_filtered_removedsites %>%
	purrr::map(~make_depth_groups(.)) %>%
	bind_rows

# 
zzz <- site_data_depthgroups %>%
	group_by(TrtID, replication_number, block, year_name, combination_group, lowest_depth) %>%
	summarise(soc_tha_lowest_depth = sum(soc_tha))
	ungroup(combination_group) %>%
	summarise(soc_low = mean(soc_tha)) %>%
